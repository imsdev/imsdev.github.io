...
H5P.Flashcards = (function ($, XapiGenerator) {
  ...
  C.prototype.getScore = function () {
    ...
    return that.options.cards.reduce(function (sum, card, i) {
      return (
        sum +
        (isCorrectAnswer(card, that.answers[i], that.options.caseSensitive)
          ? 1
          : 0)
      );
    }, 0);
  ...
  C.prototype.cardsLoaded = function () {
    ...
    var $inner = this.$container
      .html(
        '<div class="h5p-description" id="flashcards-description' +
          "-" +
          descId +
          '" title="' +
          ...
          '<div class="h5p-inner" role="region" aria-labelledby="flashcards-description' +
          "-" +
          descId +
          '" aria-roledescription="carousel"></div>' +
          '<div class="h5p-navigation">' +
          '<button type="button" class="h5p-button h5p-previous h5p-hidden" tabindex="0" title="' +
          this.options.previous +
          '" aria-label="' +
          this.options.previous +
          '"></button>' +
          '<button type="button" class="h5p-button h5p-next" tabindex="0" title="' +
          this.options.next +
          '" aria-label="' +
          this.options.next +
          '"></button>'
  ...
  C.prototype.addShowResults = function ($inner) {
    ...
    var $showResults = $(
      '<div class="h5p-show-results">' +
        ...
        '<button type="button" class="h5p-show-results-label">' +
        that.options.showResults +
        "</button>" +
        '<button type="button" class="h5p-show-results-label-mobile">' +
        that.options.results +
        "</button>" +
        "</div>"
    );
  ...
  C.prototype.addCard = function (index, $inner) {
    ...
    var $card = $(
      '<div role="group" aria-roledescription="slide" aria-labelledby="h5p-flashcard-card-' +
        cardId +
        '" class="h5p-card h5p-animate' +
        (index === 0 ? " h5p-current" : "") +
        '"> ' +
        '<div class="h5p-foot">' +
        '<div class="h5p-imagetext" id="h5p-flashcard-card-' +
        cardId +
        '">' +
        ...
        '<div class="h5p-input">' +
        '<input type="text" class="h5p-textinput" tabindex="-1" placeholder="' +
        this.options.defaultAnswerText +
        '" aria-describedby="h5p-flashcard-card-' +
        cardId +
        '" autocomplete="off" spellcheck="false"/>' +
        '<button type="button" class="h5p-button h5p-check-button" tabindex="-1" title="' +
        this.options.checkAnswerText +
        '">' +
        this.options.checkAnswerText +
        "</button>" +
        '<button type="button" class="h5p-button h5p-icon-button" tabindex="-1" title="' +
        this.options.checkAnswerText +
        '"/>' +
        "</div>" +
        "</div>" +
    ...
    $card.prepend(
      $('<div class="h5p-flashcard-overlay" tabindex="-1"></div>').on(
        "click",
        function () {
          // Set temporary focus
    ...
    var handleClick = function () {
      ...
      var userCorrect = isCorrectAnswer(
        card,
        userAnswer,
        that.options.caseSensitive
      );
      var done = false;
      ...
      if (
        !that.options.showSolutionsRequiresInput ||
        userAnswer !== "" ||
        userCorrect
      ) {
        that.numAnswered++;
        ...
        if (userCorrect) {
          $input
            ...
            .append(
              '<div class="h5p-feedback-label">' +
                that.options.correctAnswerText +
                "!</div>"
            );
          ...
          $(
            '<div class="h5p-solution">' +
              ...
              "</div>"
          ).appendTo($card.find(".h5p-imageholder"));

          that.$ariaAnnouncer.html(
            that.options.correctAnswerAnnouncement.replace(
              "@answer",
              userAnswer
            )
          );
          ...
          $input
            ...
            .addClass("h5p-wrong")
            .append(
              '<span class="h5p-feedback-label">' +
                that.options.incorrectAnswerText +
                "!</span>"
            );
          ...
          $(
            '<div class="h5p-solution">' +
              ...
              (that.options.cards[index].answer
                ? that.options.showSolutionText +
                  ": " +
                  C.splitAlternatives(that.options.cards[index].answer).join(
                    ", "
                  )
                : "") +
              "</span>" +
  ...
  C.prototype.enableResultScreen = function () {
    ...
    for (var i = 0; i < this.options.cards.length; i++) {
      ...
      //   $imageHolder.css(
      //     "background-image",
      //     'url("' + H5P.getPath(card.image.path, this.id) + '")'
      //   );
      // } else {
      ...
      $resultsAnswer.prepend(
        "<span>" + this.options.answerShortText + " </span>"
      );

      if (!userCorrect) {
        $resultsAnswer.append(
          "<span> " + this.options.showSolutionText + ": </span>"
        );
        $resultsAnswer.append(
          '<span class="h5p-correct">' +
  ...
  C.prototype.setProgress = function () {
    ...
    this.$visualProgress
      .attr(
        "aria-valuenow",
        (((index + 1) / this.options.cards.length) * 100).toFixed(2)
      )
      .find(".h5p-visual-progress-inner")
      .width(((index + 1) / this.options.cards.length) * 100 + "%");
  };
  ...
  C.prototype.setCurrent = function ($card) {
    ...
    $card.one(
      ...
      function () {
        if (
          $card.hasClass("h5p-current") &&
          !$card.find(".h5p-textinput")[0].disabled
        ) {
  ...
  C.prototype.next = function () {
    ...
    if (
      $next.is(":last-child") &&
      that.numAnswered == that.options.cards.length
    ) {
    ...
    self.$inner.children(".h5p-card").each(function () {
      ...
      if ($button.outerWidth() > $button.parent().width() * 0.4) {
        ...
        $textInput.css(
          "padding-right",
          $button.outerWidth() +
            ($textInput.parent().hasClass("has-tip") ? emSize * 2.5 : emSize)
        );
      }
    });
  ...
  C.splitAlternatives = function (text, delimiter, escaper) {
    ...
    return text.split(delimiter).map(function (element) {
